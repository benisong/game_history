...
/// </summary>
public sealed class LandOwnershipService : ILandOwnershipService
{
    public PostWarLandResolutionResult ResolveWarLandScorching(GameState state, string provinceId, string victorFactionId, bool isImperialDirectArmy)
    {
        ...
        int gentryLost = (province.GentryControlledLand * 35) / 100;
        ...
        province.ScorchedMonthsRemaining = 6;
        ...
        if (isImperialDirectArmy || victorFactionId == "court")
        {
            ...
            chronicle = $"【平乱】天子禁军与官军克复{province.Name}，兵燹之后，世家坞堡遭贼寇践踏，{gentryLost}顷兼并私田沦为无主废墟，划为战乱焦土，休耕半年。朝廷顺理成章将无主田产尽数收归国家官田！世家欲求赎回须缴纳巨资。";
        }
        else
        {
            // 军阀混战胜利方占领：战后无主地归胜方诸侯所有
            title = $"【诸侯鏖兵 · 沃野焦土】{victorFactionId}占领{province.Name}！{gentryLost}顷战乱焦土归属胜者！";
            chronicle = $"【兼并】{victorFactionId}击破敌手夺占{province.Name}，兵火连绵，境内{gentryLost}顷田亩化为焦土，休耕半年，战后归属{victorFactionId}整编。";
        }
    ...
    public void AdvanceScorchedLandMonthly(GameState state)
    {
        foreach (var (provinceId, province) in state.Provinces)
        {
            if (province.ScorchedMonthsRemaining > 0)
            {
                ...
                {
                    // 半年焦土期满：土地恢复肥力，正式转化为正常耕作土地
                    province.ScorchedLand = 0;
                    state.AddToChronicle($"【时和岁丰】{province.Name}战乱焦土休耕期满（半年），荒芜田亩重新复耕，国家有效官田全面恢复产粮！");
                }
            }
        }
    }
    ...
    public LandRepurchaseResult RepurchaseStateLandByGentry(GameState state, string provinceId, int purchaseAmount)
    {
        ...
        // 基础价格：每 1000 亩土地赎买需出资 100 万钱 (进入国库)
        ...
        int goldPaid = (maxCanPurchase / 1000) * 100;
        goldPaid = Math.Max(50, goldPaid);
    ...
    public int CollectQuarterlyLandTax(GameState state, bool applyToTreasury = true)
    {
        ...
        foreach (var (pId, p) in state.Provinces)
        {
            ...
            // 1. 国家控制土地：编户齐民，全额征税 (每1000亩贡献 10 万钱)
            int stateLandTax = (p.StateControlledLand / 1000) * 10;

            // 2. 世家控制土地：隐匿田产，少收 40% (即按 60% 征收，每1000亩贡献 6 万钱)
            int gentryLandTax = (int)((p.GentryControlledLand / 1000) * 10 * 0.60);
